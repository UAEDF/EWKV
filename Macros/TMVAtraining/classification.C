...
TString type = "ZMUMU";
TString tag = "20130618";
TString option = "_BDT_50k";
TString DYtype = "inclusive";
TString production = "2013-06-JetIDfix";
   
int main(){
   if(!exists(getTreeLocation() + "tmvaWeights/" + type + "/" + tag + option)) system("mkdir -p " + getTreeLocation + "tmvaWeights/" + type + "/" + tag + option);
   // Create a new root output file.
   TString outfileName( getTreeLocation + "tmvaWeights/" + type + "/" + tag  + option + "/TMVA.root" );
   TFile* outputFile = TFile::Open( outfileName, "RECREATE" );
   (TMVA::gConfig().GetIONames()).fWeightFileDir = getTreeLocation + "tmvaWeights/" + type + "/" + tag  + option + "/weights/";
   ...
   if(DYtype == "powheg"){
     ...
     else for(TString i : {"2","3","4"}) files["DY" + i] = new TFile("~/public/merged/EWKV/" + production + "/tmva-input/" + type + "/" + tag + "/DY" + i + ".root");
   }
