...
//  padDN->SetGridy();
  ...
  frameRatio->GetYaxis()->SetTitleSize(.1);
  frameRatio->GetYaxis()->SetLabelSize(.09);
  frameRatio->GetXaxis()->SetLabelSize(.1);
  frameRatio->GetYaxis()->SetTitleOffset(.46);
  frameRatio->GetYaxis()->SetTickLength(0.019);
  ...
  if(plotSignificance){
    ...
    if(showKS) kstext.DrawTextNDC(0.18,0.01, TString::Format("KS=%e",kolmogorov));
